...
// Per-word confidence for the panel we just OCR'd (TSV side-channel, best-effort —
// never affects parsing). Returns alphabetic words Tesseract read below the
// confidence threshold, i.e. reads worth a human's eyes (e.g. Glock vs Clock).
function ocrConfidence(server) {
  const png = p(server.toLowerCase() + '.png');
  let tsv; try { tsv = sh(TESS, [png, 'stdout', '--psm', '6', '-l', 'eng', 'tsv'], true); } catch { return []; }
  const words = [];
  for (const rowline of tsv.split(/\r?\n/)) {
...
function persist() {
  ...
  fs.writeFileSync(p('lowconf.json'), JSON.stringify(Object.values(lowconf).sort((a, b) => a.minConf - b.minConf), null, 2));
}
